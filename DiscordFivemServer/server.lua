...
-- Slay Someone
		 ...
		 TriggerEvent('chat:addMessage', t[2], {
           color = { 255, 0, 0},
           multiline = true,
          args = {"Discord Console", "^1 Je bent dood gegaan door discord console"}
         })
	     sendToDiscord("Is gekilled", "Id is dood " .. GetPlayerName(t[2]),16711680)
	 
	 
	     else
	  
       sendToDiscord("Niet vind vinden.","Deze id bestaat niet in de server. Check als dit echt de goede id is.",16711680)
...
-- Return Player List
	 ...
	 if Config.ESX then
      ...
      if count == 0 then
	     sendToDiscord("Speler lijst", "Er zijn 0 spelers online", 16711680)
	  end
	  
	 else
	 
	 sendToDiscord("Discord BOT","ESX staat niet aan in config.lua",16711680)
...
-- revive
	...
	if Config.ESX then
     ...
     if t[2] ~= nil and GetPlayerName(t[2]) ~= nil then
      ...
      sendToDiscord("Ik kan deze id niet vinden","Deze id bestaat niet meer in de server, is dit echt de goede id",16711680)
	  
	 end
	 
	else
	 
	 sendToDiscord("Discord BOT","ESX is niet aan in config.lua",16711680)
