...
while true do 
  Citizen.Wait(600000)
  PerformHttpRequest(Config.WebHook, function(err, text, headers) end, 'POST', json.encode({username = Config.ReplyUserName, content = "Discord Bot Heart Beat ontvangen ", avatar_url = Config.AvatarURL}), { ['Content-Type'] = 'application/json' })
end
end)
...
-- Get Player Count
     if string.starts(command,Config.Prefix .. "spelercount") then

       sendToDiscord("Player Counts","Huidige spelers in server : " .. GetNumPlayerIndices(),16711680)
...
-- Kick Someone
     ...
     elseif string.starts(command,Config.Prefix .. "kick") then
	     ...
	     DropPlayer(t[2],"GEKICKT VAN DISCORD CONSOLE")
	    
	 
	 
	     else
	  
	     sendToDiscord("Could Not Find","Kon geen ID vinden. Zorg ervoor dat u een geldige ID invoert",16711680)
...
-- Slay Someone
		 ...
		 TriggerEvent('chat:addMessage', t[2], {
           color = { 255, 0, 0},
           multiline = true,
          args = {"Discord Console", "^1 Je bent gedood door Discord Console"}
         })
	     sendToDiscord("DEAD Succesfully", "Succesvol gedood " .. GetPlayerName(t[2]),16711680)
...
-- Return Player List
	 ...
	 if Config.ESX then
      ...
      if count == 0 then
	     sendToDiscord("PLAYER LIST", "Er zijn 0 spelers in de server", 16711680)
	  end
	  
	 else
	 
	 sendToDiscord("Discord BOT","ESX is niet gaan gezet in de config.lua",16711680)
...
-- revive
	...
	if Config.ESX then
     ...
     if t[2] ~= nil and GetPlayerName(t[2]) ~= nil then
      ...
      sendToDiscord("Could Not Find","Kan het id niet vinden, is die persoon de server geleaved",16711680)
	  
	 end
	 
	else
	 
	 sendToDiscord("Discord BOT","ESX is niet gaan gezet in config.lua",16711680)
...
elseif string.starts(command,Config.Prefix .. "announce") then
   ...
   sendToDiscord("Bericht verzonden", "bericht verzonden naar server : " .. string.gsub(safecom, Config.Prefix .. "announce","") .. " | To " .. GetNumPlayerIndices() ..  " spelers in de server",16711680)
   
   else
   
    sendToDiscord("Niet gevonden","Verkeerd getypt",16711680)
   end

-- Command Not Found
     else

             sendToDiscord("Discord Command", "Command niet gevonden, laat het wel een goede command zijn", 16711680)
